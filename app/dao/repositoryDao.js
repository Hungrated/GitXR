    ...
    if (res && res.result && res.data) {
      ...
      try {
        if (issueRes && issueRes.result && issueRes.data) {
          ...
          netData.closed_issues_count =
            netData.all_issues_count - netData.open_issues_count
        }
      } catch (e) {
        console.log(e)
      }
...
const getRepositoryDetailReadmeHtmlDao = (userName, reposName, branch) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data.length > 0) {
      ...
      realm.write(() => {
        let data = realm
          .objects('RepositoryDetailReadme')
...
const getRepositoryForksDao = (userName, reposName, page, localNeed) => {
  ...
  let nextStep = async () => {
    let url =
      Address.getReposForks(userName, reposName) +
      Address.getPageParams('?', page)
    let res = await Api.netFetch(url)
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm
          .objects('RepositoryFork')
          .filtered(`fullName="${fullName}"`)
        realm.delete(allEvent)
        res.data.forEach(item => {
...
const getRepositoryStarDao = (userName, reposName, page, localNeed) => {
  ...
  let nextStep = async () => {
    let url =
      Address.getReposStar(userName, reposName) +
      Address.getPageParams('?', page)
    let res = await Api.netFetch(url)
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm
          .objects('RepositoryStar')
          .filtered(`fullName="${fullName}"`)
        realm.delete(allEvent)
        res.data.forEach(item => {
...
const getRepositoryWatcherDao = (userName, reposName, page, localNeed) => {
  ...
  let nextStep = async () => {
    let url =
      Address.getReposWatcher(userName, reposName) +
      Address.getPageParams('?', page)
    let res = await Api.netFetch(url)
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm
          .objects('RepositoryWatcher')
          .filtered(`fullName="${fullName}"`)
        realm.delete(allEvent)
        res.data.forEach(item => {
...
const getReposCommitsDao = (userName, reposName, page, localNeed) => {
  ...
  let nextStep = async () => {
    let url =
      Address.getReposCommits(userName, reposName) +
      Address.getPageParams('?', page)
    let res = await Api.netFetch(url)
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm
          .objects('RepositoryCommits')
          .filtered(`fullName="${fullName}"`)
        realm.delete(allEvent)
        res.data.forEach(item => {
...
const getReposCommitsInfoDao = (userName, reposName, sha) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data) {
      realm.write(() => {
        let data = realm
          .objects('RepositoryCommitInfoDetail')
...
const getUserRepository100StatusDao = async userName => {
  ...
  if (res && res.result && res.data.length > 0) {
    ...
    let myStar = await Api.netFetch(Address.userStar(userName, 'updated&per_page=1'))
    if (__DEV__) {
      console.log('fuck', JSON.stringify( myStar))
    }
    let myStarCount =0
    if (myStar && myStar.result && myStar.data.length > 0) {
      let link =  myStar.headers.map.link
       myStarCount = link.substring(
        link.lastIndexOf('page=') + 'page='.length,
        link.lastIndexOf('>')
      )
    ...
    return {
      data: {
        ...
        beStaredRepCount: await myStarCount
      },
      result: true
    }
  } else {
