...
const getTrendDao = async (page = 0, since, languageType) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allData = realm.objects('TrendRepository')
          .filtered(`since="${since}" AND languageType="${localLanguage}"`);
...
const getPulseDao = async (owner, repositoryName) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data) {
      realm.write(() => {
        let allData = realm.objects('RepositoryPulse')
          .filtered(`fullName="${fullName}"`);
        realm.delete(allData);
...
const getUserRepositoryDao = async (userName, page, sort, localNeed) => {
  ...
  let nextStep = async () => {
    let url = Address.userRepos(userName, sort) +
      Address.getPageParams('&', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('UserRepos')
          .filtered(`userName="${userName}" AND sort="${sortText}"`);
  ...
  let local = async () => {
    let allData = realm.objects('UserRepos')
      .filtered(`userName="${userName}" AND sort="${sortText}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getStarRepositoryDao = async (userName, page, sort, localNeed) => {
  ...
  let nextStep = async () => {
    let url = Address.userStar(userName, sort) +
      Address.getPageParams('&', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('UserStared')
          .filtered(`userName="${userName}" AND sort="${sortText}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('UserStared')
      .filtered(`userName="${userName}" AND sort="${sortText}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getRepositoryDetailDao = (userName, reposName) => {
  ...
  let nextStep = async () => {
    ...
    let res = await Api.netFetch(url, 'GET', null, false,
    ...
    if (res && res.result && res.data) {
      ...
      try {
        if (issueRes && issueRes.result && issueRes.data) {
          ...
          netData.closed_issues_count = netData.all_issues_count -
            netData.open_issues_count;
        }
      } catch (e) {
        console.log(e);
      }
      realm.write(() => {
        let data = realm.objects('RepositoryDetail')
          .filtered(`fullName="${fullName}" AND branch="master"`);
        realm.delete(data);
  ...
  let AllData = realm.objects('RepositoryDetail')
    .filtered(`fullName="${fullName}" AND branch="master"`);
  if (AllData && AllData.length > 0) {
    return {
      data: JSON.parse(AllData[0].data),
...
const getRepositoryDetailReadmeHtmlDao = (userName, reposName, branch) => {
  ...
  let nextStep = async () => {
    ...
    let res = await Api.netFetch(url, 'GET', null, false,
      {Accept: 'application/vnd.github.html'}, true);
    if (res && res.result && res.data.length > 0) {
      let curData = generateHtml(res.data);
      realm.write(() => {
        let data = realm.objects('RepositoryDetailReadme')
          .filtered(`fullName="${fullName}" AND branch="${curBranch}"`);
        realm.delete(data);
  ...
  let AllData = realm.objects('RepositoryDetailReadme')
    .filtered(`fullName="${fullName}" AND branch="${curBranch}"`);
  if (AllData && AllData.length > 0) {
    return {
      data: AllData[0].data,
...
const addRepositoryLocalReadDao = (userName, reposName, data) => {
  let fullName = userName + '/' + reposName;
  let allEvent = realm.objects('ReadHistory')
    .filtered(`fullName="${fullName}"`);
  realm.write(() => {
    realm.delete(allEvent);
...
const getRepositoryLocalReadDao = (page = 1) => {
  ...
  let allEvent = realm.objects('ReadHistory')
    .sorted('readDate', true)
    .slice(size, size + Config.PAGE_SIZE);
  let data = [];
...
const getBranchesDao = (userName, reposName) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data.length > 0) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryBranch')
          .filtered(`fullName="${fullName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let allData = realm.objects('RepositoryBranch')
    .filtered(`fullName="${fullName}"`);
  if (allData && allData.length > 0) {
    let data = [];
    allData.forEach((item) => {
...
const getRepositoryForksDao = (userName, reposName, page, localNeed) => {
  let fullName = userName + '/' + reposName;
  let nextStep = async () => {
    let url = Address.getReposForks(userName, reposName) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryFork')
          .filtered(`fullName="${fullName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('RepositoryFork')
      .filtered(`fullName="${fullName}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getRepositoryStarDao = (userName, reposName, page, localNeed) => {
  let fullName = userName + '/' + reposName;
  let nextStep = async () => {
    let url = Address.getReposStar(userName, reposName) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryStar')
          .filtered(`fullName="${fullName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('RepositoryStar')
      .filtered(`fullName="${fullName}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getRepositoryWatcherDao = (userName, reposName, page, localNeed) => {
  let fullName = userName + '/' + reposName;
  let nextStep = async () => {
    let url = Address.getReposWatcher(userName, reposName) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryWatcher')
          .filtered(`fullName="${fullName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('RepositoryWatcher')
      .filtered(`fullName="${fullName}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
 ...
 * 获取仓库的release列表
 */
const getRepositoryReleaseDao = async (
  userName, reposName, page, needHtml = true) => {
...
const getReposCommitsDao = (userName, reposName, page, localNeed) => {
  let fullName = userName + '/' + reposName;
  let nextStep = async () => {
    let url = Address.getReposCommits(userName, reposName) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryCommits')
          .filtered(`fullName="${fullName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('RepositoryCommits')
      .filtered(`fullName="${fullName}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getReposCommitsInfoDao = (userName, reposName, sha) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data) {
      realm.write(() => {
        let data = realm.objects('RepositoryCommitInfoDetail')
          .filtered(`fullName="${fullName}" AND sha ="${sha}"`);
        realm.delete(data);
  ...
  let AllData = realm.objects('RepositoryCommitInfoDetail')
    .filtered(`fullName="${fullName}" AND sha ="${sha}"`);
  if (AllData && AllData.length > 0) {
    return {
      data: JSON.parse(AllData[0].data),
 ...
 * 获取仓库的文件列表
 */
const getReposFileDirDao = async (
  userName, reposName, path = '', branch, text) => {
  let url = Address.reposDataDir(userName, reposName, path, branch);
  let res = await Api.netFetch(url, 'GET', null, false,
    {Accept: 'application/vnd.github.html'}, text);
  return {
    data: res.data,
    result: res.result
  };
...
const getUserRepository100StatusDao = async (userName) => {
  ...
  if (res && res.result && res.data.length > 0) {
    ...
    let myStar = Address.myStar()
    myStar = myStar.headers.map.link
    let myStarCount = myStar.substring(myStar.lastIndexOf('page=')+'page='.length,myStar.lastIndexOf('>'))
    console.log('count is ', myStarCount)
