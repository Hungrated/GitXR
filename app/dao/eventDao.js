...
const getEventReceivedDao = (page = 0, userName, localNeed) => {
  let nextStep = async () => {
    let url = Address.getEventReceived(userName) +
...
const getEventDao = (page = 0, userName, localNeed) => {
  let nextStep = async () => {
    ...
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('UserEvent')
 ...
 * 仓库活动事件
 */
const getRepositoryEventDao = async (
  page = 0, userName, repository, localNeed) => {
  let fullName = userName + '/' + repository;
  let nextStep = async () => {
    let url = Address.getReposEvent(userName, repository) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryEvent')
  ...
  let local = async () => {
    let allData = realm.objects('RepositoryEvent')
      .filtered(`fullName="${fullName}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
