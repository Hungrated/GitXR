...
const getUserInfoDao = async (userName) => {
  let nextStep = async () => {
    ...
    if (res && res.result) {
      ...
      realm.write(() => {
        let allData = realm.objects('UserInfo')
          .filtered(`userName="${res.data.login}"`);
...
const getFollowerListDao = async (userName, page, localNeed) => {
  ...
  let local = async () => {
    let allData = realm.objects('UserFollower')
      .filtered(`userName="${userName}"`);
    if (allData && allData.length > 0) {
      let data = [];
...
const getFollowedListDao = async (userName, page, localNeed) => {
  let nextStep = async () => {
    let url = Address.getUserFollow(userName) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url);
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('UserFollowed')
          .filtered(`userName="${userName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('UserFollowed')
      .filtered(`userName="${userName}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getNotifationDao = async (all, participating, page) => {
  ...
  let url = Address.getNotifation(all, participating) +
...
const getUserOrgsDao = async (userName, page, localNeed) => {
  let nextStep = async () => {
    ...
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('UserOrgs')
          .filtered(`userName="${userName}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
