 ...
 */
const getRepositoryIssueDao = (
  page = 0, userName, repository, state, sort, direction, localNeed) => {
  let fullName = userName + '/' + repository;
  let stateName = state ? state : 'all';
  let nextStep = async () => {
    let url = Address.getReposIssue(userName, repository, state, sort,
      direction) + Address.getPageParams('&', page);
    ...
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('RepositoryIssue')
          .filtered(`fullName="${fullName}" AND state="${stateName}"`);
  ...
  let local = async () => {
    let allData = realm.objects('RepositoryIssue')
      .filtered(`fullName="${fullName}" AND state="${stateName}"`);
 ...
 * issue的回复列表
 */
const getIssueCommentDao = async (
  page = 0, userName, repository, number, localNeed) => {
  let fullName = userName + '/' + repository;
  let nextStep = async () => {
    let url = Address.getIssueComment(userName, repository, number) +
      Address.getPageParams('?', page);
    let res = await Api.netFetch(url, 'GET', null, false,
      {Accept: 'application/vnd.github.html,application/vnd.github.VERSION.raw'});
    if (res && res.result && res.data.length > 0 && page <= 1) {
      realm.write(() => {
        let allEvent = realm.objects('IssueComment')
          .filtered(`fullName="${fullName}" AND number ="${number}"`);
        realm.delete(allEvent);
        res.data.forEach((item) => {
  ...
  let local = async () => {
    let allData = realm.objects('IssueComment')
      .filtered(`fullName="${fullName}" AND number ="${number}"`);
    if (allData && allData.length > 0) {
      let data = [];
      allData.forEach((item) => {
...
const getIssueInfoDao = async (userName, repository, number) => {
  ...
  let nextStep = async () => {
    ...
    if (res && res.result && res.data) {
      realm.write(() => {
        let data = realm.objects('IssueDetail')
          .filtered(`fullName="${fullName}" AND number ="${number}"`);
        realm.delete(data);
  ...
  let AllData = realm.objects('IssueDetail')
    .filtered(`fullName="${fullName}" AND number ="${number}"`);
  if (AllData && AllData.length > 0) {
    return {
...
const addIssueCommentDao = async (userName, repository, number, comment) => {
  ...
  let res = await Api.netFetch(url, 'POST', {body: comment}, true,
...
const editIssueDao = async (userName, repository, number, issue) => {
  ...
  let res = await Api.netFetch(url, 'PATCH', issue, true,
    {Accept: 'application/vnd.github.VERSION.full+json'});
  if (res && res.result) {
    realm.write(() => {
      let items = realm.objects('IssueDetail')
        .filtered(`fullName="${fullName}" AND number ="${number}"`);
...
const lockIssueDao = async (userName, repository, number, locked) => {
  ...
  let res = await Api.netFetch(url, locked ? 'DELETE' : 'PUT', {}, true,
    {Accept: 'application/vnd.github.VERSION.full+json'});
  let objectData = null;
  if (res && res.result) {
    realm.write(() => {
...
const createIssueDao = async (userName, repository, issue) => {
  ...
  let res = await Api.netFetch(url, 'POST', issue, true,
    {Accept: 'application/vnd.github.VERSION.full+json'});
  if (res && res.result) {
    realm.write(() => {
      realm.create('IssueDetail', {
 ...
 * 编辑issue回复
 */
const editCommentDao = async (
  userName, repository, number, commentId, comment) => {
  let fullName = userName + '/' + repository;
  let url = Address.editComment(userName, repository, commentId);
  let res = await Api.netFetch(url, 'PATCH', comment, true,
    {Accept: 'application/vnd.github.VERSION.full+json'});
  if (res && res.result) {
    realm.write(() => {
      let items = realm.objects('IssueComment')
        .filtered(
          `fullName="${fullName}" AND number ="${number}" AND commentId="${commentId}"`);
      if (items && items.length > 0) {
        items[0].data = JSON.stringify(res.data);
      }
    });
...
const deleteCommentDao = async (userName, repository, number, commentId) => {
  let fullName = userName + '/' + repository;
  let url = Address.editComment(userName, repository, commentId);
  let res = await Api.netFetch(url, 'DELETE', null, true,
    {Accept: 'application/vnd.github.VERSION.full+json'});
  if (res && res.result) {
    realm.write(() => {
      let items = realm.objects('IssueComment')
