...
export const generateCode2HTml = (
  mdData, backgroundColor = Constant.white, lang = 'java', userBR = true) => {
...
export const generateHtml = (
  mdData, backgroundColor = Constant.white, userBR = true) => {
  if (!mdData) {
    return '';
  }
  let mdDataCode = mdData.replace(/<code(([\s\S])*?)<\/code>/gi,
  ...
  mdDataCode = mdDataCode.replace(/<pre(([\s\S])*?)<\/pre>/gi,
    function (match, capture) {
      if (match) {
  ...
  return generateCodeHtml(data, false, backgroundColor, Constant.actionBlue,
    userBR);
};

export const generateMd2Html = (
  mdData, userName, reposName, branch = 'master', needMd = true) => {
  ...
  let data = dataPre.replace(/<pre[^>]*>([^]+)<\/pre>/gi,
    function (match, capture) {
      if (capture) {
...
const generateCodeHtml = (
  mdHTML, wrap, backgroundColor = Constant.white,
  actionColor = Constant.actionBlue, userBR = true) => {
  return '<html>\n' +
    '<head>\n' +
    '<meta charset="utf-8" />\n' +
    '<title></title>\n' +
    ...
    '<script>hljs.configure({\'useBR\': ' + userBR +
...
export const parseDiffSource = (diffSource, wrap) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    source = source + '\n' + '<div ' + classStr + '>' +
      (wrap ? '' : lineNumberStr + getBlank(1)) + line + '</div>';
  }
  return source;
};
...
export const getFullName = (repository_url) => {
  ...
  if (repository_url) {
    ...
    if (splicurl.length > 2) {
      fullName = splicurl[splicurl.length - 2] + '/' +
        splicurl[splicurl.length - 1];
    }
  }
  return fullName;
};
...
export function launchUrl (url) {
  ...
  if (parseUrl && parseUrl.hostname === 'github.com' &&
    parseUrl.pathname.length > 0) {
