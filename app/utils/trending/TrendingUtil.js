...
export default class TrendingUtil {
  static htmlToRepo (responseData) {
    ...
    for (var i = 0; i < splitWithH3.length; i++) {
      ...
      var metaNoteContent = this.parseContentWithNote(html,
        'class="f6 text-gray mt-2">', '</li>');
      repo.meta = this.parseRepoLabelWithTag(repo, metaNoteContent, TAGS.meta);
      repo.starCount = this.parseRepoLabelWithTag(repo, metaNoteContent,
        TAGS.starCount);
      repo.forkCount = this.parseRepoLabelWithTag(repo, metaNoteContent,
        TAGS.forkCount);
  ...
  static parseRepoBaseInfo (repo, htmlBaseInfo) {
    ...
    var url = htmlBaseInfo.slice(urlIndex,
    ...
    var description = this.parseContentWithNote(htmlBaseInfo,
