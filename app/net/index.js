...
class HttpManager {
  ...
  async netFetch (url, method = 'GET', params, json, header, text) {
    ...
    let response = await this.requestWithTimeout(this.optionParams.timeoutMs,
      fetch(url, requestParams), text);
    ...
    try {
      if (text) {
        ...
        if (response.status === 201 && responseJson.token) {
          ...
          AsyncStorage.setItem(Constant.TOKEN_KEY,
            this.optionParams.authorizationCode);
        }
...
export const FSModule = {
  download: (opt, callback) => NativeModules.DownloadFileModule.download(opt,
    callback),
  onProgress: (callback) => DeviceEventEmitter.addListener('DownloadStatus',
    callback),
  installAPK: (filePath) => NativeModules.DownloadFileModule.installAPK(
    filePath),
