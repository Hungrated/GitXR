...
import { ActionUtils, getActionAndDes } from '../utils/eventUtils';
...
class RepositoryDetailActivityPage extends Component {
  ...
  _renderRow (rowData) {
    if (this.state.select === 0) {
      ...
      let openStatus = this.state.pulseData.issueOpenStatus
        ? this.state.pulseData.issueOpenStatus
        : '';
      let closeStatus = this.state.pulseData.issueClosedStatus
        ? this.state.pulseData.issueClosedStatus
        : '';
  ...
  _refresh (select) {
    ...
    if (select === 0) {
      eventActions.getRepositoryEvent(0, this.props.ownerName,
      ...
      reposActions.getReposCommits(0, this.props.ownerName,
        this.props.repositoryName)
        .then((res) => {
          if (res && res.result) {
            let dataList = res.data;
  ...
  _loadMore () {
    if (this.state.select === 0) {
      eventActions.getRepositoryEvent(this.page, this.props.ownerName,
      ...
      reposActions.getReposCommits(this.page, this.props.ownerName,
        this.props.repositoryName).then((res) => {
        let size = 0;
        if (res && res.result) {
          this.page++;
  ...
  _renderHeader () {
    ...
    return <View>
      <RepositoryHeader
        ...
        repositoryIssueClose={closed_issues_count
          ? (closed_issues_count + '')
          : null}
  ...
  render () {
    let data = this.state.select === 0
      ? this.state.dataSource
      : this.state.dataSourceCommits;
