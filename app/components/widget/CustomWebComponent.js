...
import { Dimensions, Linking, Platform, WebView } from 'react-native';
    ...
    return (
      <WebCurrent
        ...
        onShouldStartLoadWithRequest={(event) => {
          ...
          } else if (event.url && event.url.indexOf('https://github.com/') ===
            0) {
            launchUrl(event.url);
          } else if (event.url &&
            (event.url.indexOf('http') === 0 || event.url.indexOf('www') ===
              0)) {
