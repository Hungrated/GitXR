...
class RepositoryDetailPage extends Component {
  ...
  componentDidMount () {
    InteractionManager.runAfterInteractions(() => {
      repositoryActions.getRepositoryDetail(this.props.ownerName,
        ...
        .then((res) => {
          if (res && res.result) {
            ...
            repositoryActions.addRepositoryLocalRead(this.props.ownerName,
              this.props.repositoryName, res.data);
      ...
      this._refresh();
      repositoryActions.getBranches(this.props.ownerName,
        this.props.repositoryName)
        .then((res) => {
          if (res && res.result) {
            this.setState({
    ...
    this.handle = BackHandler.addEventListener(
      'hardwareBackPress-RepositoryDetail', this._backHandler);
  }
  ...
  _refresh () {
    repositoryActions.getRepositoryDetailReadmeHtml(this.props.ownerName,
      this.props.repositoryName, this.curBranch)
      .then((res) => {
        if (res && res.result) {
          this.setState({
    ...
    repositoryActions.getRepositoryStatus(this.props.ownerName,
      this.props.repositoryName).then((res) => {
      if (res && res.result) {
        this.setState({
          showBottom: true,
    ...
    return [
      {
        ...
        itemClick: () => {
          Actions.LoadingModal({backExit: false});
          repositoryActions.doRepositoryStar(ownerName, repositoryName, !stared)
            .then((res) => {
              setTimeout(() => {
                Actions.pop();
                this._refresh();
        ...
        itemClick: () => {
          Actions.LoadingModal({backExit: false});
          repositoryActions.doRepositoryWatch(ownerName, repositoryName,
            !watched).then((res) => {
            setTimeout(() => {
              Actions.pop();
              this._refresh();
          ...
          borderLeftColor: Constant.lineColor,
  ...
  render () {
    let itemHeight = 30;
    let popHeight = (this.state.dataDetailBranches) ? (itemHeight *
      this.state.dataDetailBranches.length + 20) : 100;
            ...
            gsygithubLink={(url) => {
              if (url) {
                ...
                let currentPath = url.replace('gsygithub://.', '')
                  .replace('gsygithub://', '/');
                let fixedUrl = 'https://github.com/' + owner + '/' + repo +
                  '/blob/' + branch + currentPath;
                launchUrl(fixedUrl);
              }
            }}
          />
