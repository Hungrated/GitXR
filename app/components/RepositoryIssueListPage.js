...
class RepositoryIssueListPage extends Component {
  ...
  _searchText () {
    ...
    if (this.searchText === null || this.searchText.trim().length === 0) {
      issueActions.getRepositoryIssue(0, this.props.userName,
        this.props.repositoryName, this.filter)
        .then((res) => {
          if (res && res.result) {
    ...
    repositoryActions.searchRepositoryIssue(this.searchText,
      this.props.userName, this.props.repositoryName, 1, this.filter)
      .then((res) => {
        let size = 0;
        if (res && res.result) {
          this.page = 2;
  ...
  _loadMore () {
    if (this.searchText === null || this.searchText.trim().length === 0) {
      issueActions.getRepositoryIssue(this.page, this.props.userName,
        this.props.repositoryName, this.filter).then((res) => {
        let size = 0;
        if (res && res.result) {
          this.page++;
    ...
    repositoryActions.searchRepositoryIssue(this.searchText,
      this.props.userName, this.props.repositoryName, this.page, this.filter)
      .then((res) => {
        let size = 0;
        if (res && res.result) {
          this.page++;
  ...
  render () {
    let btnStyle = (Platform.OS === 'android') ? [
      styles.shadowCard, {
        width: 48,
        height: 48,
        borderRadius: 25,
    ...
    return (
      <View style={styles.mainBox}>
        ...
        <View style={[
          styles.flexDirectionRowNotFlex, styles.shadowCard, {
            backgroundColor: '#FFF',
