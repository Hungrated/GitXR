...
class SearchPage extends Component {
  ...
  _searchText (select) {
    ...
    repositoryActions.searchRepository(this.searchText, this.selectLanguageData,
      this.selectTypeData, this.selectSortData, type, 1).then((res) => {
      let size = 0;
      if (res && res.result) {
        this.page = 2;
        this.setState({
  ...
  _loadMore (select) {
    ...
    let type = (select === 0) ? null : 'user';
    repositoryActions.searchRepository(this.searchText, this.selectLanguageData,
      this.selectTypeData, this.selectSortData, type, this.page).then((res) => {
      let size = 0;
      if (res && res.result) {
        this.page++;
      ...
      <View style={styles.mainBox}>
        ...
        <View style={[
          ...
          <TouchableOpacity
            style={[
              styles.centered,
              {marginTop: 2, marginHorizontal: Constant.normalMarginEdge}]}
            onPress={() => {
              this._refresh();
            }}>
