...
import { AppState, InteractionManager, StatusBar, View } from 'react-native';
...
class DynamicPage extends Component {
  ...
  _handleAppStateChange = (nextAppState) => {
    if (this.appState.match(/inactive|background/) && nextAppState ===
      'active') {
  ...
  _refresh () {
    ...
    eventAction.getEventReceived(0, (res) => {
      ...
      setTimeout(() => {
        if (this.refs.pullList) {
          this.refs.pullList.refreshComplete(
            (res && res.length >= Config.PAGE_SIZE));
        }
      }, 500);
    });
  }
