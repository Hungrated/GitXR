...
import { BackHandler, InteractionManager, StatusBar, View } from 'react-native';
...
class CodeDetailPage extends Component {
  ...
  componentDidMount () {
    InteractionManager.runAfterInteractions(() => {
      if (this.props.needRequest) {
        reposActions.getReposFileDir(this.props.ownerName,
          this.props.repositoryName, this.props.path, this.props.branch,
          this.props.textStyle).then((res) => {
            if (res && res.result) {
              ...
              if (startLang >= 0 && endLang >= 0) {
                let tmpLang = res.data.substring(startLang + startTag.length,
                  endLang);
                if (tmpLang) {
              ...
              if ('markdown' === lang) {
                ...
                this.setState({
                  detail: generateCode2HTml(res.data,
                    Constant.webDraculaBackgroundColor, lang)
                });
              }
            } else {
              this.setState({
    ...
    this.handle = BackHandler.addEventListener(
      'CodeDetailPage-hardwareBackPress', this._BackHandler);
  }
