...
const getRepositoryIssue = async (
  page = 0, userName, repository, state, sort, direction) => {
  if (page <= 1) {
    return IssueDao.getRepositoryIssueDao(page, userName, repository, state,
      sort, direction, true);
  }
  return IssueDao.getRepositoryIssueDao(page, userName, repository, state, sort,
    direction);
};
...
const addIssueComment = async (userName, repository, number, comment) => {
  let res = await IssueDao.addIssueCommentDao(userName, repository, number,
    comment);
  if (res && res.result) {
    return {
      data: res.data,
...
const editComment = async (
  userName, repository, number, commentId, comment, type) => {
  let res;
  if (type === 'delete') {
    res = await IssueDao.deleteCommentDao(userName, repository, number,
      commentId);
  } else {
    res = await IssueDao.editCommentDao(userName, repository, number, commentId,
      comment);
  }
  if (res && res.result) {
    return {
