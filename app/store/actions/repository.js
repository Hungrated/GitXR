 ...
 */
const getTrend = (
  page = 0, since = 'daily', languageType, callback) => async (
  dispatch, getState) => {
  let resLocal = await RepositoryDao.getTrendDao(page, since, languageType,
    true);
...
const searchRepository = async (
  q, language, sort, order, type, page = 1, pageSize) => {
  if (language) {
    q = q + `%2Blanguage%3A${language}`;
  }
  let res = await RepositoryDao.searchRepositoryDao(q, sort, order, type, page,
    pageSize);
  return {
    result: res.result,
    data: res.data
...
const doRepositoryWatch = async (userName, reposName, watch) => {
  let res = await RepositoryDao.doRepositoryWatchDao(userName, reposName,
    watch);
  return {
    data: res.data,
    result: res.result
 ...
 * 获取仓库的release列表
 */
const getRepositoryRelease = async (
  userName, reposName, page = 1, needHtml = true) => {
...
const getRepositoryDetailReadme = async (userName, reposName, branch) => {
  let res = await RepositoryDao.getRepositoryDetailReadmeDao(userName,
    reposName, branch);
  if (res.result) {
...
const getRepositoryDetailReadmeHtml = async (userName, reposName, branch) => {
  let res = RepositoryDao.getRepositoryDetailReadmeHtmlDao(userName, reposName,
    branch);
  if (res.result) {
    return {
      result: true,
